...
export const storage = {
  ...
  async uploadTranscriptionJSON(filename: string, content: any) {
...
export const db = {
  ...
  // Transcriptions
  async createTranscription(transcription: Omit<Transcription, 'id' | 'created_at' | 'updated_at'>) {
    const { data, error } = await supabase
      .from('transcriptions')
  ...
  async updateTranscription(audioId: string, updates: Partial<Pick<Transcription, 'original_text' | 'edited_text'>>) {
    const { data, error } = await supabase
      .from('transcriptions')
