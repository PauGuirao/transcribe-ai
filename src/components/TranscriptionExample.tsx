...
export function TranscriptionExample() {
  ...
  const transcriptionLines = [
    { speaker: 'Logopeda', text: 'Bon dia Maria! Avui treballarem amb els sons de la "R". Primer, farem alguns exercicis de respiració per preparar la llengua i els llavis.', color: 'blue' },
    { speaker: 'Pacient', text: 'D\'acord! Ja he estat practicant a casa com em vas dir. Crec que estic millorant una mica.', color: 'green' },
    { speaker: 'Logopeda', text: 'Perfecte! Això és molt important. Ara intentarem dir aquestes paraules: "roda", "ratolí", "carrer". Recorda de posar la llengua en la posició que hem practicat.', color: 'blue' },
    { speaker: 'Pacient', text: 'Roda... ratolí... carrer. Com ho he fet?', color: 'green' },
    { speaker: 'Logopeda', text: 'Molt bé! Has millorat molt. Ara provarem amb frases més llargues. Repeteix després de mi: "El ratolí corre per la carretera".', color: 'blue' },    ]
  ...
  useEffect(() => {
    const sequence = async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 300)) // Reduced from 500ms to 100ms
      ...
      for (let i = 0; i <= transcriptionLines.length; i++) {
        setVisibleLines(i)
        await new Promise(resolve => setTimeout(resolve, 300)) // Much faster appearance
      }
      
      setAnimationState('complete')
      await new Promise(resolve => setTimeout(resolve, 5000)) // Wait longer to show the complete transcription
    ...
    sequence()
    const interval = setInterval(sequence, 12000) // Reduced from 15000ms to 12000ms
               ...
               >
                 {(animationState === 'initial' || animationState === 'dragging') && (
                   <div className="text-center text-gray-500">
                     ...
                     <p className="text-md">Arrossega el teu arxiu d'àudio aquí</p>
                   </div>
                 )}
                 ...
                 {animationState === 'processing' && (
                   <div className="text-center">
                     ...
                     />
                     <p className="text-md text-blue-600">Processant àudio...</p>
                   </div>
                 )}
               </motion.div>
                   ...
                   >
                     ...
                     >
                       <div className="flex items-center space-x-2">
                         ...
                         <span className="text-md font-medium">Sessió Pau.mp3</span>
                       </div>
                     </motion.div>
                     ...
                     {/* Cursor positioned on top of the audio file */}
                      ...
                      >
                        <path
                          ...
                          /* ↑ NOTE: we STOP before the second 'M91.616...' to avoid the hole */
                          fill="#000"
                          stroke="none"
                          fillRule="nonzero"
                        />
          ...
          {(animationState === 'transcribing' || animationState === 'complete') && (
            ...
            >
              {transcriptionLines.slice(0, visibleLines).map((line, index) => (
                ...
                >
                  <div className={`${
                    line.color === 'blue' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
              ...
              {visibleLines >= transcriptionLines.length && (
                ...
                >
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>Durada: 15:32</span>
                    <span>Paraules: 1,247</span>
                    <span>Precisió: 98.5%</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-2">
                    Transcripció automàtica amb identificació d'interlocutors
                  </p>
                </motion.div>
              )}
            </motion.div>
