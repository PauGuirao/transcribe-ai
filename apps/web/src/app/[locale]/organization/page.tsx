...
const OrganizationPageContent = React.memo(function OrganizationPageContent() {
  ...
  const validateName = (name: string) => {
    ...
    if (name.trim().length < 2) {
      setNameError(t("validation.minLength"));
      return false;
    }
    if (name.trim().length > 50) {
      setNameError(t("validation.maxLength"));
      return false;
    }
    // Check for special characters that might cause issues
    const invalidChars = /[<>:"/\\|?*]/;
    if (invalidChars.test(name.trim())) {
      setNameError(t("validation.invalidChars"));
      return false;
    }
    setNameError(null);
    return true;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || t("errors.setupError"));
      }
          ...
          <CardContent className="space-y-8 p-8">
            <div className="space-y-2 text-center">
              ...
              <p className="text-sm font-medium text-primary">
                {isGroupSetup ? t("groupSetupLabel") : t("lastStepLabel")}
              </p>
              <h2 className="text-3xl font-semibold tracking-tight text-foreground">
                {isGroupSetup ? t("groupSetupTitle") : t("regularTitle")}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isGroupSetup ? t("groupSetupDesc") : t("regularDesc")}
              </p>
              {isGroupSetup && groupInvitationData && (
                <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                  <div className="flex items-center gap-2 text-sm text-green-700 dark:text-green-300">
                    <Crown className="w-4 h-4" />
                    <span className="font-medium">{t("groupPlanActivated")}</span>
                  </div>
                </div>
              )}
            </div>
