...
import { getAuth, jsonError, jsonOk, buildWorkerUrl } from "@/lib/api-helpers"
import { TranscriptionJsonData } from "@/lib/data/transcription"
...
export async function GET(
  ...
  try {
    ...
    const { data: { session } } = await supabase.auth.getSession()
    if (!session?.access_token) {
      return jsonError("Session required", { status: 401, headers: CACHE_HEADERS.NO_CACHE })
    }
    ...
    const jsonPath = transcription.json_path as string | undefined
    ...
    // Fetch transcription JSON from R2 via Worker
    if (jsonPath) {
      try {
        const workerUrlFinal = buildWorkerUrl(jsonPath)
        ...
        if (workerResponse.ok) {
          ...
          console.log(`Failed to fetch transcription from R2: ${workerResponse.status}`)
        }
      } catch (e) {
